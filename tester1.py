...
class ImageProcessor(QThread):
    ...
    @Slot(int, int, int)
    def set_crop_parameters(self, x, y, radius):
        self.crop_params = (x, y, radius)

    def run(self):
        if self.img_path is not None:
            img = cv.imread(self.img_path)
            while True:
                x, y, radius = self.crop_params
                img_copy = img.copy()
                cv.circle(img_copy, (x, y), radius, [0, 0, 255], 2)
                crop_img = crop_trackbar(img.shape[0], img.shape[1], x, y, radius, img)
